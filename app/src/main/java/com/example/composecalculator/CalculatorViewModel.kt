...
class CalculatorViewModel : ViewModel() {
    ...
    fun onActon(action: CalculatorAction) {
        when (action) {
            is CalculatorAction.Number -> enterNumber(action.number)
            is CalculatorAction.Decimal -> enterDecimal()
            is CalculatorAction.Clear -> state = CalculatorState()
            is CalculatorAction.Operation -> enterOperation(action.operation)
            is CalculatorAction.Calculate -> calculateOperation()
            is CalculatorAction.Remove -> removeOperation()
        }
    }
    ...
    private fun calculateOperation() {
        ...
        if (number1 != null && number2 != null) {
            ...
            state = state.copy(number1 = result.toString().take(11), operation = null, number2 = "")
        }
    }
